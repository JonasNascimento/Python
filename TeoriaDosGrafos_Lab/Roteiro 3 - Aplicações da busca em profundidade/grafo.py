...
class Grafo:
    ...
    def ha_ciclo(self,raiz):
        retorno = [raiz]
        saida = self.ha_ciclo_recursivo(raiz,retorno)
        if saida[len(saida)-1] == raiz:
            return self.ha_ciclo_recursivo(raiz,retorno)

        else:
            return False

        
    def ha_ciclo_recursivo(self, raiz, retorno):
        adj = self.vertices_adjacentes(raiz)
        ares = self.arestas_sobre_vertice(raiz)

        for i in range(0, len(adj)):
                if retorno[len(retorno)-1]==retorno[0] and len(retorno) > 1:
                    return retorno
                if ares[i] not in retorno:
                    retorno.append(ares[i])
                    retorno.append(adj[i])
                    retorno = self.ha_ciclo_recursivo(adj[i], retorno)
    ...
    def caminho(self,n):
        ...
        raiz=lista_vertices[0]

        lista_caminho=self.arvore_dfs(raiz)

        x = n - 1
        for i in range(0, n+ x):
            retorno.append(lista_caminho[i])
    ...
    def conexo(self):
        for i in self.N:
            for j in self.N:
                if self.caminho_dois_vertices(i, j) == False:
                    return False
        return True
