...
class Grafo:
    ...
    def drone(self,i,f,ib,mb,points):
        import copy

        lpoints = copy.deepcopy(points)
        path = []

        battery = ib

        pnt = i
        minor = self.dijkstra(i,f)

        cont = 0

        while len(lpoints)!= 0:
            for p in lpoints:
                x = self.dijkstra(pnt,p)
                
                if len(x) <= len(minor) and len(x)-2 <= battery:
                    minor = x


            if len(minor)-2 > battery:
                return False
            ...
            battery = mb
            lpoints.remove(minor[len(minor)-1])

            pnt = minor[len(minor)-1]
            minor = self.dijkstra(pnt, f)

            cont +=1

            
        lista = self.dijkstra(path[len(path)-1],f)
        lista.pop(0)
        for i in lista:
            path.append(i)
        return path
    ...
    def dijkstra(self, u, v):
        ...
        while w!=v:
            ...
            adjw = self.vertices_adj(w)


            for r in adjw:
                b = beta[self.N.index(r)]
                if fi[self.N.index(r)]==0 and (b == 'inf' or (b > beta[self.N.index(w)]+peso)):
                    beta[self.N.index(r)] = beta[self.N.index(w)] + peso
                    pi[self.N.index(r)] = w
            ...
            menor = 'null'
            for vert in self.N:
                indx = self.N.index(vert)
                if fi[indx] == 0 and beta[indx] != 'inf':
                    if menor == 'null' or beta[indx] < menor:
                        menor = beta[indx]
                        r_ = vert
            #print(r_)
        ...
        idv = self.N.index(v)
        while(True):
            list.insert(0,pi[idv])
            idv = self.N.index(pi[idv])
            if pi[idv] == 0:
                list.append(v)
                break
